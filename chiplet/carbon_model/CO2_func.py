...
def get_bonding_yield(package_type: str, json_path: Union[str, Path] = "cfg//parameters/bonding_yield.json") -> float:
    with open(json_path, "r") as f:
        table = json.load(f)

    t = package_type.strip().lower().replace("-", "_")
    ...
    raise KeyError(f"Unknown package_type {package_type!r}. Known options include: {sorted(lookup.keys())}")

def Interposer(areas, techs, types, scaling_factors, package_type="passive", always_chiplets=False,
               interposer_node=65, tsv_pitch=0.025, ubump_pitch=0.025, hyb_pitch=0.025, tsv_size=0.005, ubump_size=0.005, hyb_size=0.005, RDLLayers=6, EMIBLayers=5, 
    ...
    router_design = 0
    #bonding_yield = 0.99
    bonding_yield = get_bonding_yield(package_type=package_type)
    #print(f"[DEBUG] ------------- Bonding yield for package_type={package_type}: {bonding_yield}") if print_info else None
    router_area =0
...
#         print(interposer_area, np.sum(areas))
        ...
        elif package_type in ['tsv', 'u_bump', 'hyb_bond']:
            dims = np.sqrt(np.array(areas, dtype=np.float64))
            if package_type == 'tsv':
                pitch = tsv_pitch
                size = tsv_size
            elif package_type == 'u_bump':
                pitch = ubump_pitch
                size = ubump_size
            elif package_type == 'hyb_bond':
                pitch = hyb_pitch
                size = hyb_size
            # num_tsv_1d = np.floor(dims/tsv_pitch) #Original 
            num_tsv_1d = np.floor(dims/pitch)
            overhead_3d = (num_tsv_1d**2) * (size**2)
...
def calculate_CO2(design, scaling_factors, techs, design_name='', num_iter=90, package_type='RDL', always_chiplets=False,
                  ...
                  interposer_node=65, rdl_layer = 6, emib_layers = 5, emib_pitch=10, tsv_pitch = 0.025, ubump_pitch = 0.025, hyb_pitch = 0.025,
                  tsv_size = 0.005, ubump_size = 0.005, hyb_size=0.005, num_beol = 8
                 ):
    ...
    for n, comb in enumerate(combinations):
        ...
        package_c, router_c, design_carbon[n,-1], router_a =Interposer(areas=design.area.values*area_scale, techs=comb, types=design.type.values,scaling_factors=scaling_factors,
                                          ...
                                          tsv_pitch=tsv_pitch,ubump_pitch=ubump_pitch,hyb_pitch=hyb_pitch, tsv_size=tsv_size, ubump_size=ubump_size, hyb_size=hyb_size, RDLLayers=rdl_layer, EMIBLayers=emib_layers, emib_pitch=emib_pitch, numBEOL=num_beol, 
